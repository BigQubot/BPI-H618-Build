	...
	case $1 in
		...
		if [[ -d "${DEB_STORAGE}" ]]; then
			...
			find "${DEB_STORAGE}" \( -name "${CHOSEN_KERNEL}_*.deb" -o \
				-name "bananapi-*.deb" -o \
				-name "plymouth-theme-bananapi_*.deb" -o \
			...
			[[ -n $RELEASE ]] && rm -f "${DEB_STORAGE}/${RELEASE}/bananapi-desktop-${RELEASE}"_*.deb
		fi
		;;
...
fingerprint_image()
{
	...
	Sources: 		https://github.com/BigQubot/BPI-H618-Build
...
display_alert "Building kernel splash logo" "$RELEASE" "info"
	...
	"${SDCARD}"/lib/firmware/bootsplash.bananapi >/dev/null 2>&1
	if [[ $BOOT_LOGO == yes || $BOOT_LOGO == desktop && $BUILD_DESKTOP == yes && $RELEASE != buster ]]; then
		[[ -f "${SDCARD}"/boot/bananapiEnv.txt ]] &&	grep -q '^bootlogo' "${SDCARD}"/boot/bananapiEnv.txt && \
		sed -i 's/^bootlogo.*/bootlogo=true/' "${SDCARD}"/boot/bananapiEnv.txt || echo 'bootlogo=true' >> "${SDCARD}"/boot/bananapiEnv.txt
...
function set_distribution_status() {
	...
	[[ "${DISTRIBUTION_STATUS}" != "supported" ]] && [[ "${EXPERT}" != "yes" ]] && exit_with_error "Banana Pi ${RELEASE} is unsupported and, therefore, only available to experts (EXPERT=yes)"
...
# Installing debian packages in the bananapi build system.
  ...
  if [[ $(dpkg --print-architecture) == amd64 ]]; then
	...
	*run after all host dependencies are installed*
	...
	if [[ ! -f "${USERPATCHES_PATH}"/README ]]; then
		...
		echo 'https://banana-pi.org' >> "${USERPATCHES_PATH}"/README
...
install_wiringpi()
{
	...
	if [[ ${IGNORE_UPDATES} != yes ]]; then
		...
		fetch_from_repo "https://github.com/BigQubot/wiringPi.git" "${EXTER}/cache/sources/wiringPi" "branch:main" "yes"
		fetch_from_repo "https://github.com/BigQubot/wiringPi-Python.git" "${EXTER}/cache/sources/wiringPi-Python" "branch:main" "yes"

	fi

	cp ${EXTER}/cache/sources/wiringPi/main ${SDCARD}/usr/src/wiringPi -rfa
	cp ${EXTER}/cache/sources/wiringPi-Python/main ${SDCARD}/usr/src/wiringPi-Python -rfa
}
