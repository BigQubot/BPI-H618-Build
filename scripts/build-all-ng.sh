...
# cleanup
rm -f /run/bananapi/*.pid
mkdir -p /run/bananapi
...
build_main ()
{
	...
	touch "/run/bananapi/${BOARD^}_${BRANCH}_${RELEASE}_${BUILD_DESKTOP}_${BUILD_MINIMAL}.pid";
	...
	echo -e "\n"
	rm "/run/bananapi/${BOARD^}_${BRANCH}_${RELEASE}_${BUILD_DESKTOP}_${BUILD_MINIMAL}.pid"
}
	...
	while read -r line; do
		...
		if [[ "${BUILD_STABILITY}" == "${STABILITY}" ]]; then
			...
			if [[ $1 != "dryrun" ]] && [[ $n -ge $START ]]; then
				...
				while :
				do
					[[ $(find /run/bananapi/*.pid 2>/dev/null | wc -l) -le ${MULTITHREAD} || -z ${MULTITHREAD} ]] && break
					sleep 5
				done
...
while :
do
		if [[ -z $(ps -uax | grep 7z | grep BananaPi) ]]; then
			break
		fi
	sleep 5
done
