...
create_board_package()
{
	...
	if [[ "${BOOTCONFIG}" != "none" ]]; then
		...
		mkdir -p "${destination}"/usr/share/bananapi/

		# create extlinux config file
		if [[ $SRC_EXTLINUX != yes ]]; then
			if [ -f "${USERPATCHES_PATH}/bootscripts/${bootscript_src}" ]; then
			  cp "${USERPATCHES_PATH}/bootscripts/${bootscript_src}" "${destination}/usr/share/bananapi/${bootscript_dst}"
			else
			  cp "${EXTER}/config/bootscripts/${bootscript_src}" "${destination}/usr/share/bananapi/${bootscript_dst}"
			fi
			[[ -n $BOOTENV_FILE && -f $SRC/config/bootenv/$BOOTENV_FILE ]] && \
				cp "${EXTER}/config/bootenv/${BOOTENV_FILE}" "${destination}"/usr/share/bananapi/bananapiEnv.txt
		fi
	...
	# Depends: fping is needed for bananapimonitor to upload bananapi-hardware-monitor.log
	...
	Suggests: bananapi-config
	Replaces: zram-config, base-files, bananapi-tools-$RELEASE, linux-${RELEASE}-root-legacy-$BOARD (<< $REVISION~), linux-${RELEASE}-root-current-$BOARD (<< $REVISION~), linux-${RELEASE}-root-next-$BOARD (<< $REVISION~)
	...
	Description: BananaPi board support files for $BOARD
	EOF
	...
	*)
	...
	[ -f "/usr/lib/bananapi/apt-updates" ] && rm /usr/lib/bananapi/apt-updates
	[ -f "/usr/lib/bananapi/firstrun-config.sh" ] && rm /usr/lib/bananapi/firstrun-config.sh
	...
	if [ remove = "\$1" ] || [ abort-install = "\$1" ]; then
	    ...
	    systemctl disable bananapi-hardware-monitor.service bananapi-hardware-optimize.service >/dev/null 2>&1
	    systemctl disable bananapi-zram-config.service bananapi-ramlog.service >/dev/null 2>&1
	...
	[ -f /etc/lib/systemd/system/bananapi-ramlog.service ] && systemctl --no-reload enable bananapi-ramlog.service
	...
	if [ -n "\$(grep -w '^ENABLED=false' /etc/default/log2ram 2> /dev/null)" ]; then
	     ...
	     sed -i "s/^ENABLED=.*/ENABLED=false/" /etc/default/bananapi-ramlog
	...
	cat <<-EOF >> "${destination}"/DEBIAN/postinst
    # move bootscript to /usr/share/bananapi
    # create a backup
    [ -f /etc/bananapi-release ] &&  . /etc/bananapi-release
    [ -z \${VERSION} ] && VERSION=$(echo \`date +%s\`)
    if [ -f /boot/$bootscript_dst ]; then
       cp /boot/$bootscript_dst /usr/share/bananapi/${bootscript_dst}-\${VERSION} >/dev/null 2>&1
       echo "NOTE: You can find previous bootscript versions in /usr/share/bananapi !"
    fi

    # cleanup old bootscript backup
    ls /usr/share/bananapi/boot.cmd-* >/dev/null 2>&1 | head -n -5 | xargs rm -f --
    ls /usr/share/bananapi/boot.ini-* >/dev/null 2>&1 | head -n -5 | xargs rm -f --

    echo "Recreating boot script"
    cp /usr/share/bananapi/$bootscript_dst /boot  >/dev/null 2>&1
    rootdev=\$(sed -e 's/^.*root=//' -e 's/ .*\$//' < /proc/cmdline)
    rootfstype=\$(sed -e 's/^.*rootfstype=//' -e 's/ .*$//' < /proc/cmdline)

    # recreate bananapiEnv.txt if it and extlinux does not exists
    if [ ! -f /boot/bananapiEnv.txt ] && [ ! -f /boot/extlinux/extlinux.conf ]; then
      cp /usr/share/bananapi/bananapiEnv.txt /boot  >/dev/null 2>&1
      echo "rootdev="\$rootdev >> /boot/bananapiEnv.txt
      echo "rootfstype="\$rootfstype >> /boot/bananapiEnv.txt
    fi
	...
	if [ ! -f "/etc/default/bananapi-motd" ]; then
		mv /etc/default/bananapi-motd.dpkg-dist /etc/default/bananapi-motd
	fi
	if [ ! -f "/etc/default/bananapi-ramlog" ] && [ -f /etc/default/bananapi-ramlog.dpkg-dist ]; then
		mv /etc/default/bananapi-ramlog.dpkg-dist /etc/default/bananapi-ramlog
	fi
	if [ ! -f "/etc/default/bananapi-zram-config" ] && [ -f /etc/default/bananapi-zram-config.dpkg-dist ]; then
		mv /etc/default/bananapi-zram-config.dpkg-dist /etc/default/bananapi-zram-config
	fi
	...
	# Reload services
	systemctl --no-reload enable bananapi-hardware-monitor.service bananapi-hardware-optimize.service bananapi-zram-config.service >/dev/null 2>&1
	exit 0
	EOF
	...
	for i in ${releases[@]}
	do
		echo "$(echo $i | sed 's/.*\///')=$(cat $i/support)" >> "${destination}"/etc/bananapi-distribution-status
	done

	# armhwinfo, firstrun, bananapimonitor, etc. config file
	cat <<-EOF > "${destination}"/etc/bananapi-release
...
POST_FAMILY_TWEAKS_BSP
	...
	fingerprint_image "${destination}/etc/bananapi.txt"
