...
compile_uboot()
{
	...
	while read -r target; do
		...
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${toolchain2}:${PATH}" \
			...
			# bananapi specifics u-boot settings
			[[ -f .config ]] && sed -i 's/CONFIG_LOCALVERSION=""/CONFIG_LOCALVERSION="-bananapi"/g' .config
	...
	Section: kernel
	Priority: optional
	Provides: bananapi-u-boot
	Replaces: bananapi-u-boot
	Conflicts: bananapi-u-boot, u-boot-sunxi
...
compile_firmware()
{
	...
	[[ $IGNORE_UPDATES != yes ]] && fetch_from_repo "https://github.com/BigQubot/firmware" "${EXTER}/cache/sources/bananapi-firmware-git" "branch:master"
	if [[ -n $FULL ]]; then
	...
	Package: bananapi-firmware${FULL}
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Installed-Size: 1
	Replaces: linux-firmware, firmware-brcm80211, firmware-ralink, firmware-samsung, firmware-realtek, bananapi-firmware${REPLACE}
	Section: kernel
	Priority: optional
	...
	# pack
	mv "bananapi-firmware${FULL}" "bananapi-firmware${FULL}_${REVISION}_all"
	display_alert "Building firmware package" "bananapi-firmware${FULL}_${REVISION}_all" "info"
	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "bananapi-firmware${FULL}_${REVISION}_all" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
	mv "bananapi-firmware${FULL}_${REVISION}_all" "bananapi-firmware${FULL}"
	rsync -rq "bananapi-firmware${FULL}_${REVISION}_all.deb" "${DEB_STORAGE}/"
...
compile_bananapi-zsh()
{

	local tmp_dir bananapi_zsh_dir
	tmp_dir=$(mktemp -d)
	chmod 700 ${tmp_dir}
	...
	# set up control file
	cat <<-END > "${tmp_dir}/${bananapi_zsh_dir}"/DEBIAN/control
	Package: bananapi-zsh
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Depends: zsh, tmux
	Section: utils
	Priority: optional
	Description: Banana Pi improved ZShell
	END

	# set up post install script
	cat <<-END > "${tmp_dir}/${bananapi_zsh_dir}"/DEBIAN/postinst
	#!/bin/sh
	...
	cp -R "${EXTER}"/cache/sources/oh-my-zsh "${tmp_dir}/${bananapi_zsh_dir}"/etc/
	cp -R "${EXTER}"/cache/sources/evalcache "${tmp_dir}/${bananapi_zsh_dir}"/etc/oh-my-zsh/plugins
	cp "${tmp_dir}/${bananapi_zsh_dir}"/etc/oh-my-zsh/templates/zshrc.zsh-template "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	chmod -R g-w,o-w "${tmp_dir}/${bananapi_zsh_dir}"/etc/oh-my-zsh/

	# we have common settings
	sed -i "s/^export ZSH=.*/export ZSH=\/etc\/oh-my-zsh/" "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	# user cache
	sed -i "/^export ZSH=.*/a export ZSH_CACHE_DIR=~\/.oh-my-zsh\/cache" "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	# define theme
	sed -i 's/^ZSH_THEME=.*/ZSH_THEME="mrtazz"/' "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	# disable prompt while update
	sed -i 's/# DISABLE_UPDATE_PROMPT="true"/DISABLE_UPDATE_PROMPT="true"/g' "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	# disable auto update since we provide update via package
	sed -i 's/# DISABLE_AUTO_UPDATE="true"/DISABLE_AUTO_UPDATE="true"/g' "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	# define default plugins
	sed -i 's/^plugins=.*/plugins=(evalcache git git-extras debian tmux screen history extract colorize web-search docker)/' "${tmp_dir}/${bananapi_zsh_dir}"/etc/skel/.zshrc

	chmod 755 "${tmp_dir}/${bananapi_zsh_dir}"/DEBIAN/postinst

	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "${tmp_dir}/${bananapi_zsh_dir}" >> "${DEST}"/${LOG_SUBPATH}/output.log 2>&1
	rsync --remove-source-files -rq "${tmp_dir}/${bananapi_zsh_dir}.deb" "${DEB_STORAGE}/"
	rm -rf "${tmp_dir}"
...
compile_plymouth-theme-bananapi()
{
	...
	plymouth_theme_bananapi_dir=bananapi-plymouth-theme_${REVISION}_all
	display_alert "Building deb" "bananapi-plymouth-theme" "info"

	mkdir -p "${tmp_dir}/${plymouth_theme_bananapi_dir}"/{DEBIAN,usr/share/plymouth/themes/bananapi}

        # set up control file
	cat <<- END > "${tmp_dir}/${plymouth_theme_bananapi_dir}"/DEBIAN/control
		Package: bananapi-plymouth-theme
		Version: $REVISION
		Architecture: all
		Maintainer: $MAINTAINER <$MAINTAINERMAIL>
		...
		Priority: optional
		Description: boot animation, logger and I/O multiplexer - bananapi theme
	END

	cp "${EXTER}"/packages/plymouth-theme-bananapi/debian/{postinst,prerm,postrm} \
		"${tmp_dir}/${plymouth_theme_bananapi_dir}"/DEBIAN/
	chmod 755 "${tmp_dir}/${plymouth_theme_bananapi_dir}"/DEBIAN/{postinst,prerm,postrm}

	#convert -resize 256x256 \
	#	"${EXTER}"/packages/plymouth-theme-bananapi/bananapi-logo.png \
	#	"${tmp_dir}/${plymouth_theme_bananapi_dir}"/usr/share/plymouth/themes/bananapi/bgrt-fallback.png

	# convert -resize 52x52 \
	#       "${EXTER}"/packages/plymouth-theme-bananapi/spinner.gif \
	#       "${tmp_dir}/${plymouth_theme_bananapi_dir}"/usr/share/plymouth/themes/bananapi/animation-%04d.png

	convert -resize 52x52 \
		"${EXTER}"/packages/plymouth-theme-bananapi/spinner.gif \
		"${tmp_dir}/${plymouth_theme_bananapi_dir}"/usr/share/plymouth/themes/bananapi/throbber-%04d.png

	cp "${EXTER}"/packages/plymouth-theme-bananapi/watermark.png \
		"${tmp_dir}/${plymouth_theme_bananapi_dir}"/usr/share/plymouth/themes/bananapi/

	cp "${EXTER}"/packages/plymouth-theme-bananapi/{bullet,capslock,entry,keyboard,keymap-render,lock}.png \
		"${tmp_dir}/${plymouth_theme_bananapi_dir}"/usr/share/plymouth/themes/bananapi/

	cp "${EXTER}"/packages/plymouth-theme-bananapi/bananapi.plymouth \
		"${tmp_dir}/${plymouth_theme_bananapi_dir}"/usr/share/plymouth/themes/bananapi/

	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "${tmp_dir}/${plymouth_theme_bananapi_dir}" > /dev/null
	rsync --remove-source-files -rq "${tmp_dir}/${plymouth_theme_bananapi_dir}.deb" "${DEB_STORAGE}/"
	rm -rf "${tmp_dir}"
}

compile_bananapi-config()
{
	local tmpdir=${SRC}/.tmp/bananapi-config_${REVISION}_all

	display_alert "Building deb" "bananapi-config" "info"


	mkdir -p "${tmpdir}"/{DEBIAN,usr/bin/,usr/sbin/,usr/lib/bananapi-config/}

	# set up control file
	cat <<-END > "${tmpdir}"/DEBIAN/control
	Package: bananapi-config
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Replaces: bananapi-bsp
	...
	Recommends: bananapi-bsp
	Suggests: libpam-google-authenticator, qrencode, network-manager, sunxi-tools
	Section: utils
	Priority: optional
	Description: Banana Pi configuration utility
	END

	install -m 755 $EXTER/cache/sources/bananapi-config/scripts/tv_grab_file $tmpdir/usr/bin/tv_grab_file
	install -m 755 $EXTER/cache/sources/bananapi-config/debian-config $tmpdir/usr/sbin/bananapi-config
	install -m 644 $EXTER/cache/sources/bananapi-config/debian-config-jobs $tmpdir/usr/lib/bananapi-config/jobs.sh
	install -m 644 $EXTER/cache/sources/bananapi-config/debian-config-submenu $tmpdir/usr/lib/bananapi-config/submenu.sh
	install -m 644 $EXTER/cache/sources/bananapi-config/debian-config-functions $tmpdir/usr/lib/bananapi-config/functions.sh
	install -m 644 $EXTER/cache/sources/bananapi-config/debian-config-functions-network $tmpdir/usr/lib/bananapi-config/functions-network.sh
	install -m 755 $EXTER/cache/sources/bananapi-config/softy $tmpdir/usr/sbin/softy
	# fallback to replace bananapi-config in BSP
	ln -sf /usr/sbin/bananapi-config $tmpdir/usr/bin/bananapi-config
...
userpatch_create()
{
	...
	if ! git diff-index --quiet --cached HEAD; then
		...
		if [[ -n $(git config user.email) ]]; then
			...
			git format-patch -1 HEAD --stdout --signature="Created with bananapi build tools https://github.com/BigQubot/BPI-H618-Build" > "${patch}"
...
overlayfs_wrapper()
{
	...
	if [[ $operation == wrap ]]; then
		local srcdir="$2"
		local description="$3"
		mkdir -p /tmp/overlay_components/ /tmp/bananapi_build/
		...
		mergeddir=$(mktemp -d --suffix="_$description" --tmpdir="/tmp/bananapi_build/")
