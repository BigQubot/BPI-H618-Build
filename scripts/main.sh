...
backtitle="Banana Pi building script, https://banana-pi.org" 
...
if [[ -z $BOARD ]]; then
	...
	options+=("bpi-m4berry"		"Allwinner H618 quad core 2GB RAM WiFi/BT GBE")
	options+=("bpi-m4zero"		"Allwinner H618 quad core 2GB RAM WiFi/BT 100M")
...
if [[ $BUILD_OPT =~ rootfs|image && -z $BUILD_DESKTOP ]]; then
	...
	# read distribution support status which is written to the bananapi-release file
...
#REPOSITORY_INSTALL="u-boot,kernel,bsp,bananapi-zsh,bananapi-config,bananapi-firmware${BUILD_DESKTOP:+,bananapi-desktop}"
...
BSP_CLI_PACKAGE_NAME="bananapi-bsp-cli-${BOARD}"
BSP_CLI_PACKAGE_FULLNAME="${BSP_CLI_PACKAGE_NAME}_${REVISION}_${ARCH}"
BSP_DESKTOP_PACKAGE_NAME="bananapi-bsp-desktop-${BOARD}"
...
CHOSEN_DESKTOP=bananapi-${RELEASE}-desktop-${DESKTOP_ENVIRONMENT}
...
# Compile u-boot if packed .deb does not exist or use the one from Banana Pi
...
# Compile kernel if packed .deb does not exist or use the one from Banana Pi
...
if [[ $BUILD_OPT == rootfs || $BUILD_OPT == image ]]; then
	...
	# Compile bananapi-config if packed .deb does not exist or use the one from Banana Pi
	if [[ ! -f ${DEB_STORAGE}/bananapi-config_${REVISION}_all.deb ]]; then
	
		[[ "${REPOSITORY_INSTALL}" != *bananapi-config* ]] && compile_bananapi-config
	fi 

	# Compile bananapi-zsh if packed .deb does not exist or use the one from repository
	if [[ ! -f ${DEB_STORAGE}/bananapi-zsh_${REVISION}_all.deb ]]; then

	        [[ "${REPOSITORY_INSTALL}" != *bananapi-zsh* ]] && compile_bananapi-zsh
	fi

	# Compile plymouth-theme-bananapi if packed .deb does not exist or use the one from repository
	if [[ ! -f ${DEB_STORAGE}/plymouth-theme-bananapi_${REVISION}_all.deb ]]; then

		[[ "${REPOSITORY_INSTALL}" != *plymouth-theme-bananapi* ]] && compile_plymouth-theme-bananapi
	fi

	# Compile bananapi-firmware if packed .deb does not exist or use the one from repository
	if [[ "${REPOSITORY_INSTALL}" != *bananapi-firmware* ]]; then

		if ! ls "${DEB_STORAGE}/bananapi-firmware_${REVISION}_all.deb" 1> /dev/null 2>&1; then
		...
		#if ! ls "${DEB_STORAGE}/bananapi-firmware-full_${REVISION}_all.deb" 1> /dev/null 2>&1; then
