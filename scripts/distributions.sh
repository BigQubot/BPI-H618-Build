...
install_common()
{
	...
	EOF
		...
		# TODO: modify $bootscript_dst or bananapiEnv.txt to make NFS boot universal
	...
	[[ "${BOOTCONFIG}" != "none" ]] && {
		if [[ "${REPOSITORY_INSTALL}" != *u-boot* ]]; then
			...
			UBOOT_VER=$(dpkg --info "${DEB_BANANAPI}/u-boot/${CHOSEN_UBOOT}_${REVISION}_${ARCH}.deb" | grep Descr | awk '{print $(NF)}')
			install_deb_chroot "${DEB_BANANAPI}/u-boot/${CHOSEN_UBOOT}_${REVISION}_${ARCH}.deb" "bananapi"
		fi
	}
...
PRE_INSTALL_KERNEL_DEBS
	...
	[[ -n $KERNELSOURCE ]] && {
		if [[ "${REPOSITORY_INSTALL}" != *kernel* ]]; then
			...
			VER=$(dpkg --info "${DEB_BANANAPI}/${CHOSEN_KERNEL}_${REVISION}_${ARCH}.deb" | grep Descr | awk '{print $(NF)}')
			VER="${VER/-$LINUXFAMILY/}"

			install_deb_chroot "${DEB_BANANAPI}/${CHOSEN_KERNEL}_${REVISION}_${ARCH}.deb" "bananapi"

			if [[ -f ${DEB_BANANAPI}/${CHOSEN_KERNEL/image/dtb}_${REVISION}_${ARCH}.deb ]]; then
				install_deb_chroot "${DEB_BANANAPI}/${CHOSEN_KERNEL/image/dtb}_${REVISION}_${ARCH}.deb" "bananapi"
			fi

			if [[ $INSTALL_HEADERS == yes ]]; then
				install_deb_chroot "${DEB_BANANAPI}/${CHOSEN_KERNEL/image/headers}_${REVISION}_${ARCH}.deb" "bananapi"
			fi
		fi
	}
...
POST_INSTALL_KERNEL_DEBS
	...
	if [[ "${REPOSITORY_INSTALL}" != *bsp* ]]; then
		...
		install_deb_chroot "${DEB_BANANAPI}/$RELEASE/${CHOSEN_ROOTFS}_${BSP_CLI_PACKAGE_FULLNAME}.deb" "bananapi"
	fi

	# install bananapi-desktop
	if [[ "${REPOSITORY_INSTALL}" != *bananapi-desktop* ]]; then
		...
		if [[ $BUILD_DESKTOP == yes ]]; then
			install_deb_chroot "${CHOSEN_DESKTOP}" "bananapi"
			# install display manager and PACKAGE_LIST_DESKTOP_FULL packages if enabled per board
			desktop_postinstall
		fi
	fi

	# install bananapi-firmware
	if [[ "${REPOSITORY_INSTALL}" != *bananapi-firmware* ]]; then
		if [[ -f ${DEB_STORAGE}/bananapi-firmware_${REVISION}_all.deb ]]; then
			install_deb_chroot "${DEB_STORAGE}/bananapi-firmware_${REVISION}_all.deb"
		fi
	else
		if [[ -f ${DEB_STORAGE}/bananapi-firmware_${REVISION}_all.deb ]]; then
			install_deb_chroot "${DEB_BANANAPI}/bananapi-firmware_${REVISION}_all.deb" "bananapi"
		fi
	fi

	# install bananapi-config
	if [[ "${PACKAGE_LIST_RM}" != *bananapi-config* ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *bananapi-config* ]]; then
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_STORAGE}/bananapi-config_${REVISION}_all.deb"
			fi
		else
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_BANANAPI}/bananapi-config_${REVISION}_all.deb" "bananapi"
			fi
		fi
	fi

	# install bananapi-zsh
	if [[ "${PACKAGE_LIST_RM}" != *bananapi-zsh* ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *bananapi-zsh* ]]; then
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_STORAGE}/bananapi-zsh_${REVISION}_all.deb"
			fi
		else
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "bananapi-zsh" "remote"
			fi
		fi
	fi

	# install plymouth-theme-bananapi
	if [[ $PLYMOUTH == yes && $BUILD_DESKTOP == yes && $RELEASE != buster ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *plymouth-theme-bananapi* ]]; then
			install_deb_chroot "${DEB_STORAGE}/bananapi-plymouth-theme_${REVISION}_all.deb"
		else
			install_deb_chroot "bananapi-plymouth-theme" "remote"
		fi
	fi
	...
	# add bananapi user
	chroot "${SDCARD}" /bin/bash -c "adduser --quiet --disabled-password --shell /bin/bash --home /home/${BPI_USERNAME} --gecos ${BPI_USERNAME} ${BPI_USERNAME}"
	chroot "${SDCARD}" /bin/bash -c "(echo ${OPI_PWD};echo ${OPI_PWD};) | passwd "${BPI_USERNAME}" >/dev/null 2>&1"
	for additionalgroup in sudo netdev audio video disk tty users games dialout plugdev input bluetooth systemd-journal ssh; do
	        chroot "${SDCARD}" /bin/bash -c "usermod -aG ${additionalgroup} ${BPI_USERNAME} 2>/dev/null"
	done

	# fix for gksu in Xenial
	touch ${SDCARD}/home/${BPI_USERNAME}/.Xauthority
	chroot "${SDCARD}" /bin/bash -c "chown ${BPI_USERNAME}:${BPI_USERNAME} /home/${BPI_USERNAME}/.Xauthority"
	# set up profile sync daemon on desktop systems
	chroot "${SDCARD}" /bin/bash -c "which psd >/dev/null 2>&1"
	if [ $? -eq 0 ]; then
		echo -e "${BPI_USERNAME} ALL=(ALL) NOPASSWD: /usr/bin/psd-overlay-helper" >> ${SDCARD}/etc/sudoers
		touch ${SDCARD}/home/${BPI_USERNAME}/.activate_psd
		chroot "${SDCARD}" /bin/bash -c "chown $BPI_USERNAME:$BPI_USERNAME /home/${BPI_USERNAME}/.activate_psd"
	fi
	...
	cp "${EXTER}"/packages/blobs/splash/bananapi-u-boot.bmp "${SDCARD}"/boot/boot.bmp
	...
	[[ -f "${SDCARD}"/usr/bin/gnome-session ]] && sed -i "s/user-session.*/user-session=ubuntu-wayland/" ${SDCARD}/etc/lightdm/lightdm.conf.d/22-bananapi-autologin.conf
	[[ -f "${SDCARD}"/usr/bin/startplasma-x11 ]] && sed -i "s/user-session.*/user-session=plasma-x11/" ${SDCARD}/etc/lightdm/lightdm.conf.d/22-bananapi-autologin.conf
...
FAMILY_TWEAKS
	...
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-firstrun.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-firstrun-config.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-zram-config.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-hardware-optimize.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-ramlog.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-resize-filesystem.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable bananapi-hardware-monitor.service >/dev/null 2>&1"
	...
	# save initial bananapi-release state
	cp "${SDCARD}"/etc/bananapi-release "${SDCARD}"/etc/bananapi-image-release
...
install_distribution_specific()
{
	...
	bionic|focal|hirsute|impish|jammy)
			...
			[[ -d "${SDCARD}"/etc/netplan ]] && cat <<-EOF > "${SDCARD}"/etc/netplan/bananapi-default.yaml
			network:
			  version: 2
			  renderer: $RENDERER
			EOF
...
install_opi_specific()
{
	...
	install -m 755 "${EXTER}/packages/raspi/bananapi/common/files/hciattach_bpi" "${SDCARD}/usr/bin/"
