...
create_desktop_package ()
{
	...
	Section: xorg
	Priority: optional
	Recommends: ${DEBIAN_RECOMMENDS//[:space:]+/,}, bananapi-bsp-desktop
	Provides: ${CHOSEN_DESKTOP}, bananapi-${RELEASE}-desktop
	Pre-Depends: ${PACKAGE_LIST_PREDEPENDS//[:space:]+/,}
	Description: Banana Pi desktop for ${DISTRIBUTION} ${RELEASE}
	EOF
	...
	# BananaPi create_desktop_package scripts

	unset aggregated_content

	mkdir -p "${destination}"/etc/bananapi

	local aggregated_content=""
	aggregate_all_desktop "bananapi/create_desktop_package.sh" $'\n'
...
create_bsp_desktop_package ()
{
	...
	# set up control file
	cat <<-EOF > "${destination}"/DEBIAN/control
	Package: bananapi-bsp-desktop-${BOARD}
	Version: $REVISION
	Architecture: $ARCH
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Installed-Size: 1
	Section: xorg
	Priority: optional
	Provides: bananapi-bsp-desktop, bananapi-bsp-desktop-${BOARD}
	Depends: ${BSP_CLI_PACKAGE_NAME}
	Description: Banana Pi Board Specific Packages for desktop users using $ARCH ${BOARD} machines
	EOF
	...
	local aggregated_content=""
	aggregate_all_desktop "debian/bananapi-bsp-desktop/postinst" $'\n'
	...
	# Banana Pi create_desktop_package scripts
	...
	local aggregated_content=""
	aggregate_all_desktop "debian/bananapi-bsp-desktop/prepare.sh" $'\n'
	eval "${aggregated_content}"
...
desktop_postinstall ()
{
	...
	if [[ $LINUXFAMILY == sun8i && $BRANCH == legacy ]]; then
		...
		echo "disp_mem_reserves=on" >> "${SDCARD}"/boot/bananapiEnv.txt
		echo "extraargs=cma=96M" >> "${SDCARD}"/boot/bananapiEnv.txt
	fi

	if [[ -d ${SDCARD}/etc/lightdm ]]; then
		mkdir -p ${SDCARD}/etc/lightdm/lightdm.conf.d
		cat <<-EOF > ${SDCARD}/etc/lightdm/lightdm.conf.d/22-bananapi-autologin.conf
		[Seat:*]
		autologin-user=$BPI_USERNAME
