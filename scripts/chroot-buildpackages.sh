...
create_build_script ()
{
	...
	# set local version
	# debchange -l~bananapi${REVISION}-${builddate}+ "Custom $VENDOR release"
	debchange -l~bananapi"${REVISION}"+ "Custom $VENDOR release"
...
chroot_installpackages_local()
{
	...
	if [[ $EXTERNAL_NEW == prebuilt ]]; then
		aptly -config="${conf}" repo add temp "${DEB_BANANAPI}/extra/${RELEASE}-desktop/" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
		aptly -config="${conf}" repo add temp "${DEB_BANANAPI}/extra/${RELEASE}-utils/" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
	else
	...
	cat <<-'EOF' > "${SDCARD}"/etc/apt/preferences.d/90-bananapi-temp.pref
	Package: *
	Pin: origin "localhost"
	Pin-Priority: 550
	EOF
	cat <<-EOF > "${SDCARD}"/etc/apt/sources.list.d/bananapi-temp.list
...
chroot_installpackages()
{
	...
	rm /etc/apt/sources.list.d/bananapi-temp.list 2>/dev/null
	rm /etc/apt/preferences.d/90-bananapi-temp.pref 2>/dev/null
	...
	[[ -f ${SDCARD}/etc/hostapd.conf ]] && sed -i "s/^ssid=.*/ssid=BananaPi/" ${SDCARD}/etc/hostapd.conf
} #############################################################################
